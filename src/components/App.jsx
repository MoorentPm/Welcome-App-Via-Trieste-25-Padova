...
import { IconHome, IconMap, IconSparkle, IconMsg, IconCheck } from './Icons'
import Login, { loadGuest, saveGuest, clearGuest } from './Login'
import HomeChiavi from './Home'
import Padova from './Padova'
import {
  Itinerary, Host, Checkin, Checkout, Wifi, House, Appliance,
...
export default function App() {
  ...
  const [guest, setGuest] = React.useState(() => loadGuest())
  ...
  switch (route.name) {
    ...
    case 'settings':       screen = <Settings back={back} guest={guest}
                             onLogout={() => { clearGuest(); setGuest(null) }}
                             onSave={(g) => { setGuest(g); saveGuest(g) }} />; break
  ...
  return (
    <div style={{
      ...
      <TweakRow label="Stato soggiorno">
        {[{ v: 'pre', l: 'Pre-arrivo' }, { v: 'stay', l: 'Soggiorno' }, { v: 'out', l: 'Ultimo dì' }].map(o => (
