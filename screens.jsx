// screens.jsx — All detail screens: itinerary (swipe), checkin, wifi, house, places, checkout, review, host

const AP = window.APARTMENT;
...
window.loadItineraries = function() {
  try { return JSON.parse(localStorage.getItem(ITIN_STORAGE) || "[]"); } catch { return []; }
};
window.saveItinerary = function(itin) {
  const list = window.loadItineraries();
...
window.deleteItinerary = function(id) {
  const list = window.loadItineraries().filter(i => i.id !== id);
  try { localStorage.setItem(ITIN_STORAGE, JSON.stringify(list)); } catch {}
  return list;
};

function SavedItineraries({ onOpen }) {
  const [items, setItems] = React.useState(() => window.loadItineraries());
  const [confirmDel, setConfirmDel] = React.useState(null);
  const del = (id, e) => { e?.stopPropagation(); setItems(window.deleteItinerary(id)); setConfirmDel(null); };
...
function ItineraryResult({ mood, days, loading, back, go, reopen }) {
...
function ItineraryDays({ mood, plans, days, back, go, reopen }) {
  ...
  const onSave = () => {
    if (saved) return;
    window.saveItinerary({ mood, days });
    setSaved(true);
  };
...
// ─────────────────────────────────────────────────────
function ArrivalCheckin({ back, go }) {
  const steps = window.CHECKIN_STEPS;
...
// ─────────────────────────────────────────────────────
function TransportDetail({ back, mode }) {
  const data = {
    train: {
...
// ─────────────────────────────────────────────────────
function NavBar({ back, title, right }) {
  return (
    <div style={{
...
function House({ back, go }) {
  ...
  const coTotal = window.CHECKOUT_STEPS.length;
  ...
  return (
    <div className="screen-scroll page-enter" style={{ paddingBottom: 110 }}>
      ...
      <CollapsibleSection title="Regole della casa" sub={`${window.HOUSE_RULES.length} regole · da leggere all'arrivo`}
      icon="📜" open={!!open.rules} onToggle={() => toggle("rules")}>
        <div className="card-tight" style={{ background: "transparent", boxShadow: "none" }}>
          {window.HOUSE_RULES.map((r, i) =>
          <div key={i} className="row">
              <div style={{ fontSize: 22, flexShrink: 0, width: 32, textAlign: "center" }}>{r.icon}</div>
              <div className="grow">
                <div className="t-15 w-600">{r.t}</div>
      ...
      <CollapsibleSection title="Regole di check-out" sub={`${window.CHECKOUT_STEPS.length} passaggi · prima di andare`}
...
// ─────────────────────────────────────────────────────
function Appliance({ back, item }) {
  const a = item || window.APPLIANCES[0];
  return (
    <div className="screen-scroll page-enter" style={{ paddingBottom: 110 }}>
...
// CHECKOUT
// ─────────────────────────────────────────────────────
function Checkout({ back, go }) {
  const steps = window.CHECKOUT_STEPS;
...
function Host({ back, guest }) {
  ...
  const systemContext = () => {
    const rules = window.HOUSE_RULES.map((r) => `• ${r.t}: ${r.d}`).join("\n");
    const appliances = window.APPLIANCES.map((a) => `• ${a.t}: ${a.desc}`).join("\n");
...
// ─────────────────────────────────────────────────────
function Coupons({ back, go, guest }) {
...
// ─────────────────────────────────────────────────────
function CouponDetail({ back, item, guest }) {
  const c = item || window.COUPONS_RICH[0];
  const lang = guest?.lang || "it";
...
// ─────────────────────────────────────────────────────
function PlaceDetail({ back, place }) {
  const p = place || window.PLACES[0];
  return (
    <div className="screen-scroll page-enter" style={{ paddingBottom: 110 }}>
      <div style={{ position: "relative" }}>
...
Object.assign(window, {
  Itinerary, Checkin, Wifi, House, Places, Checkout, Review, Host, Arrival, PlaceDetail, NavBar,
  Appliance, About, CollapsibleSection, ArrivalCheckin, Coupons, TransportDetail, CouponDetail, DailyTip
});